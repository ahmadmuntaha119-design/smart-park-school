<x-admin-layout>
    ...
    ============================================================ --}}
    <div class="mb-5">
        <h2 class="text-xl sm:text-2xl font-extrabold tracking-tight" style="color: var(--on-s);">Pusat Kontrol Kendaraan Siswa</h2>
        <p class="text-xs sm:text-sm font-medium mt-0.5" style="color: var(--on-v);">Assign zona massal, filter data, dan kelola kendaraan siswa.</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6" x-data="{ showHapus: false }">
        ...
        <form action="{{ route('admin.kendaraan.bulkAssign') }}" method="POST" id="formBulkAssign"
              class="rounded-2xl p-5 sm:p-6 overflow-hidden relative border"
            ...
            <div class="absolute -right-6 -bottom-6 opacity-[0.03] pointer-events-none" aria-hidden="true">
                <span class="material-icons text-[110px]" style="color: var(--on-s);">map</span>
            </div>

            <div class="relative z-10">
                <p class="text-[10px] font-extrabold tracking-widest uppercase mb-1" style="color: var(--on-v);">Aksi Massal</p>
                <h3 class="text-base sm:text-lg font-extrabold mb-0.5" style="color: var(--on-s);">Assign Zona & Baris Massal</h3>
                <p class="text-xs font-medium mb-4" style="color: var(--on-v);">Centang motor · pilih zona · pilih baris spesifik · klik Assign.</p>

                <div class="space-y-2.5">
                    {{-- Pilih Zona --}}
                    <select name="id_zona" required @change="loadBaris($event.target.value)"
                            class="w-full px-4 py-3 rounded-xl font-bold text-sm appearance-none cursor-pointer transition-colors"
                            style="background: rgba(255,255,255,0.02); border: 1px solid var(--outline-v); color: var(--on-s); outline: none;">
                        <option value="" disabled selected class="text-black bg-white">Pilih Zona Tujuan</option>
                        @foreach($semuaZona as $zona)
                            <option value="{{ $zona->id }}" class="text-black bg-white">Zona {{ $zona->nama_zona }} (Sisa: {{ $zona->sisa_kuota }})</option>
                        @endforeach
                    </select>
                    ...
                    <div x-show="barisList.length > 0" x-transition style="display:none;">
                        <select name="id_baris" required
                                ...
                                class="w-full px-4 py-3 rounded-xl font-bold text-sm appearance-none cursor-pointer transition-colors"
                                style="background: rgba(255,255,255,0.02); border: 1px solid var(--outline-v); color: var(--on-s); outline: none;">
                            <option value="" class="text-black bg-white">— Pilih Baris Parkir —</option>
                            <template x-for="b in barisList" :key="b.id">
                                <option :value="b.id" class="text-black bg-white"
                    ...
                    <div x-show="barisTerpilih" x-transition style="display:none;"
                        ...
                        <p class="text-[10px] font-bold uppercase tracking-widest mb-1" style="color:var(--on-v);">INFO PETAK OTOMATIS</p>
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-sm font-extrabold" style="color:var(--on-s);" x-text="'Petak ' + barisTerpilih?.slot_awal + ' – ' + barisTerpilih?.slot_akhir"></p>
                                <p class="text-[11px] font-semibold" style="color:var(--on-v);"
                                   x-text="barisTerpilih?.sisa + ' petak kosong dari ' + barisTerpilih?.kapasitas"></p>
                            </div>
                            <div class="text-right">
                                <p class="text-[10px] font-bold uppercase" style="color:var(--on-v);">Akan Diisi</p>
                                <p class="text-sm font-extrabold" style="color:var(--on-s);"
                                   x-text="jumlahDicentang + ' motor → Petak ' + barisTerpilih?.slot_awal"></p>
                            </div>
                        </div>
                        {{-- Peringatan aturan --}}
                        <template x-if="barisTerpilih?.aturan_label">
                            <div class="mt-2 rounded-lg px-3 py-2 flex items-center gap-2 border"
                                 style="background:rgba(234,179,8,0.1); border-color:rgba(234,179,8,0.2);">
                                <span class="material-icons text-[14px] text-yellow-500">warning</span>
                                <p class="text-[10px] font-bold text-yellow-500">
                    ...
                    <button type="submit"
                            class="w-full px-5 py-3 rounded-xl font-extrabold text-sm tracking-widest uppercase transition-colors"
        ...
        {{-- ---- Panel Hapus Kelas ---- --}}
        <div class="rounded-2xl p-5 sm:p-6 border relative overflow-hidden" style="background: var(--sl2); border-color: var(--outline-v);">
            <div class="absolute -right-4 -bottom-4 opacity-[0.03] pointer-events-none" aria-hidden="true">
                <span class="material-icons text-[100px]" style="color: var(--on-s);">school</span>
            </div>
            <div class="relative z-10">
                <div class="flex items-center gap-2 mb-0.5">
                    <span class="material-icons text-[18px] text-red-500">warning_amber</span>
                    <h3 class="text-sm font-extrabold" style="color: var(--on-s);">Hapus Data Berdasarkan Kelas</h3>
                </div>
                <br>

                <div x-show="!showHapus">
                    <button @click="showHapus = true"
                            class="w-full py-3 rounded-xl text-sm font-bold transition-colors border border-dashed"
                            style="border-color: rgba(239,68,68,0.5); color: #ef4444;"
                            onmouseenter="this.style.background='rgba(239,68,68,0.05)'"
                ...
                <form action="{{ route('admin.kendaraan.hapusAngkatan') }}" method="POST"
                    ...
                    <div class="flex gap-2">
                        <div class="relative flex-1">
                            <input type="text" name="nama_kelas" required
                                   ...
                                   class="block w-full px-3 py-3 rounded-xl text-sm font-bold transition-colors"
                                   style="background: rgba(239,68,68,0.05); border: 1px solid rgba(239,68,68,0.2); color: #ef4444; outline: none;"
                                   onfocus="this.style.borderColor='rgba(239,68,68,0.5)'"
                                   onblur="this.style.borderColor='rgba(239,68,68,0.2)'">
                        </div>
                        <button type="submit"
                                class="px-4 py-3 rounded-xl text-xs font-extrabold text-white uppercase tracking-wider whitespace-nowrap transition-colors"
                    ...
                    <p class="text-[12px] font-semibold mt-2 text-red-500">
    ...
    ============================================================ --}}
    <div x-data="kendaraanTable()" class="rounded-2xl border overflow-hidden" style="background: var(--sl2); border-color: var(--outline-v);">

        {{-- Table Top Bar --}}
        <div class="px-4 sm:px-6 py-4 border-b" style="border-color: var(--outline-v); background: transparent;">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                <div>
                    <h3 class="text-sm sm:text-base font-extrabold" style="color: var(--on-s);">Seluruh Data Kendaraan</h3>
                    <p class="text-xs font-medium mt-0.5" style="color: var(--on-v);"><span x-text="visibleCount">0</span> motor ditampilkan</p>
                </div>
                {{-- Global Search --}}
                <div class="relative w-full sm:w-72">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <span class="material-icons text-[18px]" style="color: var(--on-v);">search</span>
                    </div>
                    <input type="text" x-model="searchQuery" @input="applyFilters()"
                           ...
                           onblur="this.style.borderColor='var(--outline-v)'">
                </div>
            </div>
            ...
            <div class="flex flex-wrap gap-2 mt-3">
                ...
                <select x-model="filterMerek" @change="applyFilters()"
                        ...
                        style="background: rgba(255,255,255,0.02); border: 1px solid var(--outline-v); color: var(--on-s); outline: none;">
                    <option value="" class="text-black bg-white">Semua Merek</option>
                    @foreach($mereks as $m)
                        <option value="{{ $m->nama_merek }}" class="text-black bg-white">{{ $m->nama_merek }}</option>
                    @endforeach
                </select>
                ...
                <select x-model="filterWarna" @change="applyFilters()"
                        class="text-xs font-bold px-3 py-2 rounded-xl cursor-pointer transition-colors"
                        style="background: rgba(255,255,255,0.02); border: 1px solid var(--outline-v); color: var(--on-s); outline: none;">
                    <option value="" class="text-black bg-white">Semua Warna</option>
                    @foreach($warnaList as $w)
                        <option value="{{ $w }}" class="text-black bg-white">{{ $w }}</option>
                    @endforeach
                </select>
                ...
                <select x-model="filterTransmisi" @change="applyFilters()"
                        class="text-xs font-bold px-3 py-2 rounded-xl cursor-pointer transition-colors"
                        style="background: rgba(255,255,255,0.02); border: 1px solid var(--outline-v); color: var(--on-s); outline: none;">
                    <option value="" class="text-black bg-white">Matic & Manual</option>
                    <option value="Matic" class="text-black bg-white">Matic</option>
                    <option value="Manual" class="text-black bg-white">Manual</option>
                </select>
                ...
                <select x-model="filterKelas" @change="applyFilters()"
                    ...
                    @foreach($kendaraans->pluck('kelas')->unique()->sort() as $kelas)
                        <option value="{{ $kelas }}" class="text-black bg-white">{{ $kelas }}</option>
                    @endforeach
                </select>
                ...
                <select x-model="filterZona" @change="applyFilters()"
                        class="text-xs font-bold px-3 py-2 rounded-xl cursor-pointer transition-colors"
                        style="background: rgba(255,255,255,0.02); border: 1px solid var(--outline-v); color: var(--on-s); outline: none;">
                    <option value="" class="text-black bg-white">Semua Zona</option>
                    <option value="belum" class="text-black bg-white">Belum Ada Zona</option>
                    @foreach($semuaZona as $z)
                        <option value="{{ $z->nama_zona }}" class="text-black bg-white">Zona {{ $z->nama_zona }}</option>
                    @endforeach
                </select>

                {{-- Reset Filter --}}
                <button @click="resetFilters()" x-show="searchQuery || filterMerek || filterWarna || filterTransmisi || filterKelas || filterZona"
                        class="text-xs font-extrabold px-3 py-2 rounded-xl flex items-center gap-1 transition-colors"
                        style="background: rgba(255,255,255,0.05); color: var(--on-v); border: 1px solid rgba(255,255,255,0.1);"
                        onmouseenter="this.style.background='rgba(255,255,255,0.1)'; this.style.color='var(--on-s)'"
                        onmouseleave="this.style.background='rgba(255,255,255,0.05)'; this.style.color='var(--on-v)'">
            ...
            <div class="max-h-[540px] overflow-y-auto custom-scrollbar">
                <table id="tabelKendaraan" class="w-full text-sm text-left min-w-[900px]">
                    <thead class="text-[10px] uppercase tracking-widest sticky top-0 z-10"
                           style="color: var(--on-v); background: rgba(255,255,255,0.02); border-bottom: 1px solid var(--outline-v); backdrop-filter: blur(8px);">
                        <tr>
                            <th class="px-4 py-3.5 w-10 text-center">
                                <input type="checkbox" id="checkAll" class="w-4 h-4 rounded cursor-pointer"
                                       style="accent-color: var(--on-s);"
                    ...
                    <tbody style="color: var(--on-s);">
                        @forelse($kendaraans as $kendaraan)
                            <tr class="border-b baris-data transition-colors"
                                ...
                                <td class="px-4 py-3 font-semibold text-xs" style="color: var(--on-v);">{{ $kendaraan->kelas }}</td>
                                <td class="px-4 py-3 font-semibold text-xs uppercase" style="color: var(--on-v);">
                                    {{ $kendaraan->merek->nama_merek ?? '-' }} {{ $kendaraan->model_motor }}
                                </td>
                                <td class="px-4 py-3">
                                    @if($kendaraan->warna)
                                        <span class="text-xs font-bold px-2 py-1 rounded-lg" style="background:rgba(255,255,255,0.05); color:var(--on-s); border:1px solid var(--outline-v);">{{ $kendaraan->warna }}</span>
                                    @else
                                        <span class="text-xs" style="color:var(--on-v);">-</span>
                                    @endif
                                </td>
                                <td class="px-4 py-3">
                                    @if($kendaraan->jenis_transmisi)
                                        <span class="text-xs font-extrabold px-2 py-1 rounded-lg"
                                              style="background: {{ $kendaraan->jenis_transmisi === 'Matic' ? 'rgba(16,185,129,0.1)' : 'rgba(59,130,246,0.1)' }}; color: {{ $kendaraan->jenis_transmisi === 'Matic' ? '#10b981' : '#3b82f6' }};">
                                ...
                                <td class="px-4 py-3">
                                    @if($kendaraan->zona)
                                        <div>
                                            ...
                                            @if($kendaraan->baris || $kendaraan->nomor_slot)
                                                <span class="block text-[10px] font-bold mt-1" style="color:var(--on-v);">
                                                    {{ $kendaraan->baris->nama_baris ?? '' }}
                                                    @if($kendaraan->nomor_slot)
                                                        <span class="px-1.5 py-0.5 rounded text-[9px] text-black font-extrabold ml-0.5 bg-white">NO: {{ $kendaraan->nomor_slot }}</span>
                                                    @endif
                                                </span>
                                            @endif
                                        </div>
                                    @else
                                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-extrabold"
                                              style="background: rgba(255,255,255,0.05); color: var(--on-v); border: 1px solid rgba(255,255,255,0.1);">
                                            <span class="w-1.5 h-1.5 rounded-full animate-pulse bg-zinc-500"></span>
                                            Belum Ada
                                        </span>
                                    @endif
                                </td>
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-end gap-1.5">
                                        ...
                                        <button type="button"
                                                ...
                                                style="border-color: var(--outline-v); color: var(--on-v); background: transparent;"
                                                onmouseenter="this.style.borderColor='var(--on-s)'; this.style.color='var(--on-s)'; this.style.background='rgba(255,255,255,0.05)'"
                                                onmouseleave="this.style.borderColor='var(--outline-v)'; this.style.color='var(--on-v)'; this.style.background='transparent'"
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="py-20 text-center">
                                    <span class="material-icons text-[56px] block mx-auto mb-3 opacity-50" style="color: var(--on-v);">two_wheeler</span>
                                    <p class="font-extrabold text-lg" style="color: var(--on-v);">Belum Ada Kendaraan Terdaftar</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        {{-- MOBILE: Card List --}}
        <div class="md:hidden divide-y custom-scrollbar" style="border-color: var(--outline-v); max-height: 60vh; overflow-y: auto;">
            @forelse($kendaraans as $kendaraan)
                <div class="p-4 baris-data transition-colors"
                     style="border-color: var(--outline-v);"
                     data-nis="{{ strtolower($kendaraan->user->nis_nip ?? '') }}"
                     data-nama="{{ strtolower($kendaraan->user->nama_lengkap ?? '') }}"
                     data-kelas="{{ $kendaraan->kelas }}"
                    ...
                    <div class="flex items-start justify-between gap-3">
                        <div class="flex items-center gap-3">
                            <input type="checkbox" name="kendaraan_ids[]" value="{{ $kendaraan->id }}"
                                   ...
                                   style="accent-color: var(--on-s);">
                            <div>
                                <p class="font-extrabold text-sm leading-tight" style="color: var(--on-s);">{{ $kendaraan->user->nama_lengkap ?? '-' }}</p>
                                <p class="text-xs font-bold mt-0.5" style="color: var(--on-v);">NIS {{ $kendaraan->user->nis_nip ?? '-' }} · {{ $kendaraan->kelas }}</p>
                                <p class="text-xs font-semibold mt-1 uppercase" style="color: var(--on-v);">
                                    {{ $kendaraan->merek->nama_merek ?? '-' }} {{ $kendaraan->model_motor }}
                                    · <span class="font-extrabold tracking-widest text-white">{{ $kendaraan->plat_nomor }}</span>
                                </p>
                            </div>
                        </div>
                        <div class="flex flex-col items-end gap-2 flex-shrink-0">
                            ...
                            @else
                                <span class="text-[9px] font-extrabold px-2 py-1 rounded-lg border"
                                      style="background: rgba(255,255,255,0.05); color: var(--on-v); border-color:rgba(255,255,255,0.1);">Belum Ada</span>
                            @endif
                            <div class="flex gap-1">
                                <button type="button"
                                        @click="bukaEditModal({{ $kendaraan->id }}, '{{ $kendaraan->plat_nomor }}', '{{ $kendaraan->id_zona ?? '' }}', '{{ $kendaraan->kelas }}', '{{ $kendaraan->id_merek }}', '{{ $kendaraan->model_motor }}', '{{ $kendaraan->id_baris ?? '' }}', '{{ $kendaraan->nomor_slot ?? '' }}')"
                                        class="p-1.5 rounded-lg border transition-colors" style="border-color: var(--outline-v); color: var(--on-v);"
                                        onmouseenter="this.style.background='rgba(255,255,255,0.05)'; this.style.color='var(--on-s)';"
                                        onmouseleave="this.style.background='transparent'; this.style.color='var(--on-v)';">
                                ...
                                <form action="{{ route('admin.kendaraan.destroy', $kendaraan->id) }}" method="POST"
                                    ...
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="p-1.5 rounded-lg border transition-colors" style="border-color: rgba(239,68,68,0.2); color: #ef4444;"
            ...
            @empty
                <div class="py-16 text-center">
                    <span class="material-icons text-[48px] block mx-auto mb-3 opacity-50" style="color: var(--on-v);">two_wheeler</span>
                    <p class="font-extrabold" style="color: var(--on-v);">Belum Ada Kendaraan</p>
                </div>
            @endforelse
        </div>
    ...
    <div x-data="{ open: false, aksi_url: '', plat: '', zona: '', kelas: '', merek: '', tipe: '', baris: '', nmslot: '' }"
        ...
        <div @click.away="open = false"
             ...
             class="w-full sm:max-w-lg rounded-t-3xl sm:rounded-2xl overflow-hidden shadow-2xl max-h-[90vh] overflow-y-auto border"
             style="background: var(--sl2); border-color: var(--outline-v);">

            {{-- Modal Header --}}
            <div class="px-6 py-4 border-b flex items-center justify-between sticky top-0" style="background: var(--sl2); border-color: var(--outline-v); z-index:20;">
                <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-xl flex items-center justify-center" style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1);">
                        <span class="material-icons text-[20px]" style="color: var(--on-s);">edit_note</span>
                    </div>
                    <div>
                        <h3 class="text-sm font-extrabold" style="color: var(--on-s);">Edit Data Kendaraan</h3>
                        <p class="text-xs font-medium" style="color: var(--on-v);">Koreksi data atau pindah zona individu</p>
                    </div>
                </div>
                <button @click="open = false" class="p-2 rounded-xl transition-colors" style="color: var(--on-v);"
            ...
            <div class="p-5 sm:p-6">
                <form :action="aksi_url" method="POST" class="space-y-4">
                    ...
                    <div class="space-y-1.5">
                        <label class="block text-xs font-extrabold uppercase tracking-wider" style="color: var(--on-s);">Kelas Siswa</label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <span class="material-icons text-[18px]" style="color: var(--on-v);">class</span>
                            </div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div class="space-y-1.5">
                            <label class="block text-xs font-extrabold uppercase tracking-wider" style="color: var(--on-s);">Merek</label>
                            <select name="id_merek" x-model="merek" required
                                    ...
                                    onfocus="this.style.borderColor='rgba(255,255,255,0.3)'"
                                    onblur="this.style.borderColor='var(--outline-v)'">
                                <option value="" class="text-black bg-white">— Pilih —</option>
                                @foreach($mereks as $m)
                                    <option value="{{ $m->id }}" class="text-black bg-white">{{ $m->nama_merek }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="space-y-1.5">
                            <label class="block text-xs font-extrabold uppercase tracking-wider" style="color: var(--on-s);">Tipe Motor</label>
                    ...
                    {{-- Plat Nomor --}}
                    <div class="space-y-1.5">
                        <label class="block text-xs font-extrabold uppercase tracking-wider" style="color: var(--on-s);">Plat Nomor</label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <span class="material-icons text-[18px]" style="color: var(--on-v);">pin</span>
                            </div>
                    ...
                    {{-- Zona --}}
                    <div class="space-y-1.5">
                        <label class="block text-xs font-extrabold uppercase tracking-wider" style="color: var(--on-s);">Zona Parkir</label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <span class="material-icons text-[18px]" style="color: var(--on-v);">place</span>
                            </div>
                            <select name="id_zona" x-model="zona"
                                    ...
                                    onfocus="this.style.borderColor='rgba(255,255,255,0.3)'"
                                    onblur="this.style.borderColor='var(--outline-v)'">
                                <option value="" class="text-black bg-white">— Kosongkan Zona —</option>
                                @foreach($semuaZona as $z)
                                    <option value="{{ $z->id }}" class="text-black bg-white">{{ $z->nama_zona }} (Sisa: {{ $z->sisa_kuota }})</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    {{-- Nomor Slot --}}
                    <div class="space-y-1.5">
                        <label class="block text-xs font-extrabold uppercase tracking-wider" style="color: var(--on-s);">Nomor Slot (Opsional)</label>
                    ...
                    {{-- Footer Actions --}}
                    <div class="flex gap-3 pt-3 border-t" style="border-color: var(--outline-v);">
                        ...
                        <button type="submit"
                                class="flex-1 py-3 rounded-xl text-sm font-extrabold tracking-wider uppercase transition-colors"
                                style="background: white; color: black; box-shadow: 0 4px 12px rgba(255,255,255,0.2);"
                                onmouseenter="this.style.background='#e4e4e7'"
                                onmouseleave="this.style.background='white'">
                            Simpan Perubahan
                        </button>
